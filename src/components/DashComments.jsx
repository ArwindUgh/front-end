...
export default function DashComments() {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      setIsLoading(true);
      try {
        const res = await fetch('https://back-end-ys5e.onrender.com/api/comment/getcomments');
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await fetch(`https://back-end-ys5e.onrender.com/api/comment/getcomments?startIndex=${startIndex}`);
      const data = await res.json();
      if (res.ok) {
  ...
  const handleDeleteComment = async () => {
    setShowModal(false);
    try {
      const res = await fetch(`https://back-end-ys5e.onrender.com/api/comment/deleteComment/${commentIdToDelete}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      const data = await res.json();
