...
export default function Comment({ comment, onLike, onEdit, onDelete }) {
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      try {
        const res = await fetch(`https://v-api-hd0j.onrender.com/api/user/${comment.userId}`);
  ...
  const handleSave = async () => {
    try {
      const res = await fetch(`https://v-api-hd0j.onrender.com/api/comment/editComment/${comment._id}`, {
        method: 'PUT',
        headers: {
